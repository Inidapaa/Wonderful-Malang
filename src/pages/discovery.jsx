...
import { Search, MapPin, Tag, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DiscoveryPage() {
  ...
  const fetchItems = async (opts = {}) => {
    const { text = search, kat = kategori, kid = kecamatanId, pg = page } = opts;
    setLoading(true);
    let q = supabase
      .from("wisata")
      .select(
    ...
    if (text) {
      ...
      q = q.or(
        `nama_wisata.ilike.%${text}%,deskripsi.ilike.%${text}%`
      );
    }
  ...
  const openDetail = async (w) => {
    ...
    const [peng, kec] = await Promise.all([
      ...
      w?.id_kecamatan
        ? supabase
            .from("kecamatan")
            .select("id, nama_kecamatan, alamat, deskripsi, jumlah_wisata, kode_pos")
    ...
    <div className="min-h-screen w-full bg-primary overflow-y-scroll no-scrollbar pt-20 transition duration-200">
      ...
      <section className="w-full bg-primary text-white">
        <div className="max-w-6xl mx-auto px-6 py-14 md:py-20">
          ...
          <p className="mt-3 md:mt-4 text-white/90 max-w-2xl">
            Jelajahi ragam wisata alam, budaya, dan kuliner. Pilih destinasi dan
            lihat detailnya secara instan.
          </p>
        </div>
      </section>
      ...
      <section className="w-full">
        <div className="max-w-6xl mx-auto px-6 pb-16">
          {/* Controls - styled like provided design */}
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem key="__ALL__" value="__ALL__">Semua Kecamatan</SelectItem>
                      {kecList.map((k) => (
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem key="__ALL__" value="__ALL__">Semua Kategori</SelectItem>
                      {kategoriList.map((k) => (
                  ...
                  <div className="absolute inset-x-0 bottom-0 z-10 p-4 text-white">
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <div className="text-[11px] text-white/80">
                        Kecamatan: <span className="font-medium text-white">{kecMap[it.id_kecamatan]?.nama_kecamatan || "-"}</span>
                      </div>
                      <Button
                        size="sm"
        ...
        <div className="fixed inset-0 z-300 flex items-center justify-center backdrop-blur-2xl transition-all">
          ...
          <div className="relative z-10 w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="grid grid-cols-1 md:grid-cols-5">
              ...
              {/* Right: Content */}
              <div className="md:col-span-3 p-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
                ...
                {selected.wisata.lokasi && (
                  <div className="text-sm text-gray-700">
                    <span className="font-medium">Lokasi:</span> {selected.wisata.lokasi}
                  </div>
                )}
                <div className="text-gray-700 text-sm leading-relaxed bg-gray-50 rounded-xl p-3 border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border">
                    <h4 className="font-semibold text-primary mb-2">Pengelola</h4>
                    {selected.pengelola ? (
                      <dl className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-sm text-gray-700">
                        ...
                        <dd className="line-clamp-2">{selected.pengelola.alamat || "-"}</dd>
                        <dt className="font-medium">Deskripsi</dt>
                        <dd className="line-clamp-3">{selected.pengelola.deskripsi || "-"}</dd>
                      </dl>
                    ) : (
                      <div className="text-sm text-gray-500">Tidak ada data pengelola</div>
                    )}
                  </div>

                  {/* Kecamatan */}
                  <div className="bg-gray-50 rounded-xl p-4 border">
                    <h4 className="font-semibold text-primary mb-2">Kecamatan</h4>
                    {selected.kecamatan ? (
                      <dl className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-sm text-gray-700">
                        <dt className="font-medium">Nama</dt>
                        <dd>{selected.kecamatan.nama_kecamatan || "-"}</dd>
                        <dt className="font-medium">Alamat</dt>
                        <dd className="line-clamp-2">{selected.kecamatan.alamat || "-"}</dd>
                        <dt className="font-medium">Kode Pos</dt>
                        <dd>{selected.kecamatan.kode_pos || "-"}</dd>
                        <dt className="font-medium">Deskripsi</dt>
                        <dd className="line-clamp-3">{selected.kecamatan.deskripsi || "-"}</dd>
                        <dt className="font-medium">Jumlah Wisata</dt>
                        <dd>{selected.kecamatan.jumlah_wisata ?? "-"}</dd>
                      </dl>
                    ) : (
                      <div className="text-sm text-gray-500">Tidak ada data kecamatan</div>
                    )}
                  </div>
                </div>
